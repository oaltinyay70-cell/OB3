...
class _ScenarioWizardScreenState extends State<ScenarioWizardScreen> {
  ...
  Future<void> _init() async {
    try {
      ...
      if (widget.editScenarioId != null) {
        ...
        // Default: ALL cards selected with qty=1 (full deck)
        _data.targetCards = { for (final c in _allTargetCards) c['id'] as int: 1 };
        _data.threatCards = { for (final c in _allThreatCards) c['card_number'] as String: 1 };
        _data.combatCards = { for (final c in _allCombatCards) c['id'] as int: 1 };
      }
      ...
      // Prevent late initialization error if it fails
      _data = DesignerScenarioData();
    }
  }

  Future<void> _save({bool publish = false}) async {
    // Pre-validate before publishing — show clean dialog if issues found
    if (publish) {
          ...
          builder: (ctx) => AlertDialog(
            ...
            title: Row(
              children: [
                ...
                const Text('Cannot Publish', style: TextStyle(color: Colors.amber, fontSize: 16)),
              ],
            ),
            content: Column(
    ...
    try {
      // Auto-increment version on every save (+0.01)
      _data.version = double.parse((_data.version + 0.01).toStringAsFixed(2));
  ...
  /// Export current scenario data as a .txt file (browser download).
  Future<void> _exportScenario() async {
    try {
      final content = _data.toOb3Scenario();
      final safeName = _data.title.isNotEmpty
          ? _data.title.replaceAll(RegExp(r'[^a-zA-Z0-9_]'), '_').toLowerCase()
          : 'untitled';
      final fileName = '${safeName}_v${_data.version.toStringAsFixed(2)}.txt';
      downloadBytes(fileName, Uint8List.fromList(utf8.encode(content)));

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Exported "$fileName" — check your Downloads folder'),
  ...
  /// Pick a local image file for the briefing visual.
  Future<void> _pickBriefingImage() async {
    try {
      ...
      // Validate size: max 100 KB
      if (bytes.length > 100 * 1024) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Image too large (${(bytes.length / 1024).toStringAsFixed(0)} KB). Max 100 KB.'),
      ...
      setState(() {
        _data.missionBriefingImagePath = 'data:$mimeType;base64,$b64';
      });
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _imageErrorPlaceholder() {
    return Container(
      ...
      child: const Text('⚠ Image could not be loaded', style: TextStyle(color: Colors.amber)),
    );
  }
  ...
  Widget build(BuildContext context) {
    if (_loading || _status.startsWith('Error:')) {
      return Scaffold(
        backgroundColor: AppColors.background,
        appBar: AppBar(backgroundColor: const Color(0xFF1A1A2E),
            title: Text(_status.startsWith('Error:') ? 'Error' : 'Loading...', style: const TextStyle(color: Colors.amber))),
        body: Center(
          child: _status.startsWith('Error:') 
              ? Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    const Icon(Icons.error_outline, color: Colors.red, size: 48),
                    const SizedBox(height: 16),
                    Text(_status, style: const TextStyle(color: Colors.red, fontFamily: 'Courier')),
                    const SizedBox(height: 16),
                    ElevatedButton(
        ...
        title: Row(
          children: [
            ...
            Container(
              ...
              child: Text('v${_data.version.toStringAsFixed(2)}',
                  style: const TextStyle(color: Colors.amber, fontSize: 11, fontFamily: 'Courier')),
            ),
          ],
        ),
        actions: [
          // Export as .ob3scenario text file (available on all 9 steps)
          IconButton(
            icon: const Icon(Icons.file_download, color: Colors.amber, size: 20),
            tooltip: 'Export as .ob3scenario file',
          ...
          TextButton(
            ...
            child: const Text('PUBLISH', style: TextStyle(color: Colors.greenAccent, fontSize: 14)),
          ),
        ],
      ),
      body: Column(
  ...
  Widget _buildBasicInfo() {
    return Column(
      ...
      children: [
        ...
        // ── Briefing Visual (file upload) ──
        const SizedBox(height: 8),
        Text("BRIEFING VISUAL", style: AppTextStyles.label),
        const SizedBox(height: 4),
        Text(
          'Upload an image displayed above the mission briefing. '
          'Accepted formats: JPG, PNG. Max size: 100 KB. '
        ...
        Row(
          children: [
            ElevatedButton.icon(
              ...
              label: Text(
                _data.missionBriefingImagePath != null ? 'Replace Image' : 'Upload Image',
              ...
              onPressed: _pickBriefingImage,
            ),
            if (_data.missionBriefingImagePath != null) ...[
              const SizedBox(width: 12),
              TextButton(
                onPressed: () => setState(() => _data.missionBriefingImagePath = null),
                child: const Text('Remove', style: TextStyle(color: Colors.redAccent, fontSize: 12)),
              ),
            ],
          ],
        ),
        if (_data.missionBriefingImagePath != null && _data.missionBriefingImagePath!.isNotEmpty)
          Container(
            ...
            child: _data.missionBriefingImagePath!.startsWith('data:')
                ? Image.memory(
                    base64Decode(_data.missionBriefingImagePath!.split(',').last),
                    height: 180,
                    width: double.infinity,
                    fit: BoxFit.cover,
                ...
                : Image.network(
                    _data.missionBriefingImagePath!,
                    height: 180,
                    width: double.infinity,
                    fit: BoxFit.cover,
                    errorBuilder: (_, __, ___) => _imageErrorPlaceholder(),
        ...
        // Primary condition
        _buildObjectiveConditions(
          label: 'PRIMARY',
          color: AppColors.danger,
          vpEnabled: _data.primaryVpEnabled,
          onVpEnabledChanged: (v) => setState(() => _data.primaryVpEnabled = v),
          killCountEnabled: _data.primaryKillCountEnabled,
          onKillCountEnabledChanged: (v) => setState(() => _data.primaryKillCountEnabled = v),
          killTargetsEnabled: _data.primaryKillTargetsEnabled,
          onKillTargetsEnabledChanged: (v) => setState(() => _data.primaryKillTargetsEnabled = v),
          killConditions: _data.primaryKillConditions,
          vpThreshold: _data.primaryObjectiveVpThreshold,
          onVpChanged: (v) => setState(() => _data.primaryObjectiveVpThreshold = int.tryParse(v)),
          targetCount: _data.primaryObjectiveTargetCount,
          onTargetCountChanged: (v) => setState(() => _data.primaryObjectiveTargetCount = int.tryParse(v)),
          onKillConditionsChanged: () => setState(() {}),
        ),

        _textField('Secondary Objective', _data.secondaryObjective, (v) => _data.secondaryObjective = v, maxLen: 500, lines: 2),

        // Secondary condition (same structure as primary)
        _buildObjectiveConditions(
          label: 'SECONDARY',
          color: AppColors.info,
          vpEnabled: _data.secondaryVpEnabled,
          onVpEnabledChanged: (v) => setState(() => _data.secondaryVpEnabled = v),
          killCountEnabled: _data.secondaryKillCountEnabled,
          onKillCountEnabledChanged: (v) => setState(() => _data.secondaryKillCountEnabled = v),
          killTargetsEnabled: _data.secondaryKillTargetsEnabled,
          onKillTargetsEnabledChanged: (v) => setState(() => _data.secondaryKillTargetsEnabled = v),
          killConditions: _data.secondaryKillConditions,
          vpThreshold: _data.secondaryObjectiveVpThreshold,
          onVpChanged: (v) => setState(() => _data.secondaryObjectiveVpThreshold = int.tryParse(v)),
          targetCount: _data.secondaryObjectiveTargetCount,
          onTargetCountChanged: (v) => setState(() => _data.secondaryObjectiveTargetCount = int.tryParse(v)),
          onKillConditionsChanged: () => setState(() {}),
          vpBonus: _data.secondaryObjectiveVpBonus,
          onVpBonusChanged: (v) => setState(() => _data.secondaryObjectiveVpBonus = double.tryParse(v) ?? 0.0),
        ),
        const SizedBox(height: 12),
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // OBJECTIVE CONDITIONS BUILDER
  // ═══════════════════════════════════════════════════════════════════════════

  /// Builds the multi-condition objective panel (used by both primary & secondary).
  Widget _buildObjectiveConditions({
    required String label,
    required Color color,
    required bool vpEnabled,
    required ValueChanged<bool> onVpEnabledChanged,
    required bool killCountEnabled,
    required ValueChanged<bool> onKillCountEnabledChanged,
    required bool killTargetsEnabled,
    required ValueChanged<bool> onKillTargetsEnabledChanged,
    required List<KillCondition> killConditions,
    required int? vpThreshold,
    required ValueChanged<String> onVpChanged,
    required int? targetCount,
    required ValueChanged<String> onTargetCountChanged,
    required VoidCallback onKillConditionsChanged,
    double? vpBonus,
    ValueChanged<String>? onVpBonusChanged,
  }) {
    // Extract unique categories from loaded target cards
    final categories = _allTargetCards
    ...
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('$label CONDITION — check boxes to enable',
              style: AppTextStyles.label.copyWith(fontSize: 10, color: color)),
          const SizedBox(height: 4),
          Text('Only checked conditions are active. Enable one, some, or all.',
              style: AppTextStyles.caption),
          const SizedBox(height: 8),
          ...
          Row(
            children: [
              SizedBox(
                ...
                child: Checkbox(
                  value: vpEnabled,
                  onChanged: (v) => onVpEnabledChanged(v ?? false),
                  activeColor: color,
              ...
              Expanded(
                child: Opacity(
                  opacity: vpEnabled ? 1.0 : 0.35,
                  child: IgnorePointer(
                    ignoring: !vpEnabled,
                    child: _textField(
                      'Min VP',
                      vpThreshold?.toString() ?? '',
                      onVpChanged,
                      number: true,
                    ),
                  ),
                ...
                Expanded(
                  child: Opacity(
                    ...
                    child: IgnorePointer(
                      ...
                      child: _textField(
                        'Bonus VP',
                        vpBonus != null && vpBonus > 0 ? vpBonus.toStringAsFixed(1) : '',
                        onVpBonusChanged,
                        number: true,
                      ),
                    ),
          ...
          Row(
            children: [
              SizedBox(
                width: 24, height: 24,
                child: Checkbox(
                  value: killCountEnabled,
                  onChanged: (v) => onKillCountEnabledChanged(v ?? false),
                  activeColor: color,
                  side: BorderSide(color: color.withValues(alpha: 0.5)),
                ),
              ),
              ...
              Expanded(
                child: Opacity(
                  opacity: killCountEnabled ? 1.0 : 0.35,
                  child: IgnorePointer(
                    ignoring: !killCountEnabled,
                    child: _textField(
                      'Min Kill Count',
          ...
          Row(
            children: [
              SizedBox(
                width: 24, height: 24,
                child: Checkbox(
                  value: killTargetsEnabled,
                  onChanged: (v) => onKillTargetsEnabledChanged(v ?? false),
                  activeColor: color,
                  side: BorderSide(color: color.withValues(alpha: 0.5)),
                ),
              ),
              const SizedBox(width: 6),
              Text('KILL TARGETS', style: AppTextStyles.label.copyWith(fontSize: 10, color: killTargetsEnabled ? color : AppColors.textSecondary)),
            ],
          ),
          const SizedBox(height: 4),
          Opacity(
            opacity: killTargetsEnabled ? 1.0 : 0.35,
            child: IgnorePointer(
              ignoring: !killTargetsEnabled,
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
          ...killConditions.asMap().entries.map((entry) {
            final idx = entry.key;
            final kc = entry.value;
            // Cards filtered by this condition's category
            final cardsInCategory = _allTargetCards
                .where((c) => c['sub_category'] == kc.category)
                .toList();
            return Padding(
              padding: const EdgeInsets.only(bottom: 6),
              child: Row(
                children: [
                  ...
                  Expanded(
                    ...
                    child: Container(
                      ...
                      child: DropdownButton<String>(
                        value: categories.contains(kc.category) ? kc.category : null,
                        hint: Text('Category', style: TextStyle(color: Colors.amber.withValues(alpha: 0.5), fontSize: 11)),
                        isExpanded: true,
                        dropdownColor: AppColors.surfaceElevated,
                        underline: const SizedBox.shrink(),
                        style: const TextStyle(color: AppColors.textPrimary, fontSize: 12, fontFamily: 'Courier'),
                        ...
                        onChanged: (v) {
                          if (v != null) {
                            kc.category = v;
                            final first = _allTargetCards.firstWhere(
                              (c) => c['sub_category'] == v,
                              orElse: () => _allTargetCards.first,
                            );
                            kc.cardNumber = first['card_number'] as String;
                            kc.cardName = first['card_name'] as String;
                            onKillConditionsChanged();
                          }
                        },
                      ),
                    ),
                  ...
                  Expanded(
                    ...
                    child: Container(
                      ...
                      child: DropdownButton<String>(
                        value: cardsInCategory.any((c) => c['card_number'] == kc.cardNumber)
                            ? kc.cardNumber
                            : null,
                        hint: Text('Select card', style: TextStyle(color: Colors.amber.withValues(alpha: 0.5), fontSize: 11)),
                        isExpanded: true,
                        dropdownColor: AppColors.surfaceElevated,
                        underline: const SizedBox.shrink(),
                        style: const TextStyle(color: AppColors.textPrimary, fontSize: 12, fontFamily: 'Courier'),
                        items: cardsInCategory.map((c) {
                          final cn = c['card_number'] as String;
                          final name = c['card_name'] as String;
                        ...
                        onChanged: (v) {
                          if (v != null) {
                            final card = cardsInCategory.firstWhere((c) => c['card_number'] == v);
                            kc.cardNumber = v;
                            kc.cardName = card['card_name'] as String;
                            onKillConditionsChanged();
                          }
                        },
                      ),
                      ...
                      children: [
                        InkWell(
                          ...
                          child: Container(
                            ...
                            child: Icon(Icons.remove, size: 14, color: kc.quantity > 1 ? color : AppColors.textSecondary),
                          ),
                        ),
                        Container(
                          width: 20, height: 28,
                          decoration: BoxDecoration(color: AppColors.surface, border: Border.all(color: color.withValues(alpha: 0.3))),
                          child: Center(child: Text('${kc.quantity}', style: TextStyle(color: color, fontSize: 13, fontWeight: FontWeight.bold))),
                        ),
                        InkWell(
                          onTap: () {
                            kc.quantity++;
          ...
          // Add kill target button
          const SizedBox(height: 4),
          InkWell(
            onTap: () {
              final firstCat = categories.isNotEmpty ? categories.first : 'TANK';
              final firstCard = _allTargetCards.firstWhere(
                (c) => c['sub_category'] == firstCat,
                orElse: () => _allTargetCards.first,
              );
              killConditions.add(KillCondition(
                cardNumber: firstCard['card_number'] as String,
                cardName: firstCard['card_name'] as String,
                category: firstCat,
                quantity: 1,
              ));
              onKillConditionsChanged();
            ...
            child: Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 4),
                  Text('ADD KILL TARGET', style: TextStyle(color: color, fontSize: 11, fontWeight: FontWeight.bold)),
                ],
              ),
            ),
          ),
