...
class _ScenarioListTabState extends State<ScenarioListTab> {
  ...
  Future<void> _importFile() async {
    try {
      ...
      // On web, use bytes; on desktop, use path
      final file = result.files.single;
      String content;
      if (file.bytes != null) {
        content = utf8.decode(file.bytes!);
      } else if (file.path != null) {
        // Desktop fallback — should not happen on web
        content = 'ERROR: File reading not supported on this platform';
      } else {
        widget.onOutput('Cannot read the selected file');
        return;
      }
      final importResult = await ScenarioImportExport.importScenario(content);
      final data = importResult.data;
      widget.onOutput('Imported "${data.title}" from ${file.name}');
  ...
  Future<void> _exportTemplate() async {
    try {
      ...
      await FilePicker.platform.saveFile(
        ...
        fileName: 'scenario_template.ob3scenario',
        type: FileType.any,
        bytes: bytes,
      );
      widget.onOutput('Template exported');
    } catch (e) {
  ...
  Future<void> _exportDatabase() async {
    try {
      final bytes = await DatabaseService.getDatabaseBytes();
      if (bytes == null) {
        widget.onOutput('Exporting raw DB is not fully supported on this platform without web or file saving access.');
        return;
      }
      downloadBytes('ob3_export.db', bytes);
      widget.onOutput('Exported database successfully');
    } catch (e) {
      widget.onOutput('Failed to export DB: $e');
    }
  }
  ...
  Future<void> _export(ScenarioListItem item) async {
    try {
      ...
      final fileName = '${safeName}_v${item.version.toStringAsFixed(2)}.txt';
      downloadBytes(fileName, Uint8List.fromList(utf8.encode(content)));
      widget.onOutput('Exported "$fileName" — check your Downloads folder');
    } catch (e) {
            ...
            children: [
              ...
              PopupMenuButton<String>(
                ...
                itemBuilder: (_) => [
                  ...
                  const PopupMenuItem(value: 'export_db', child: Text('Download Database (.db)', style: TextStyle(color: Colors.amber, fontWeight: FontWeight.bold))),
                ],
              ),
            ],
          ),
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const Spacer(),
              Text('v${item.version.toStringAsFixed(2)}', style: AppTextStyles.body),
            ],
          ),
          const SizedBox(height: 8),
          // Row 3: Actions
